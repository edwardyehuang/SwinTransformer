# Copyright 2021 https://github.com/edwardyehuang. All Rights Reserved.

import numpy as np
import tensorflow as tf

class Mlp(tf.keras.Model):
    def __init__(self, in_features, hidden_features=None, out_features = None, dropout_rate = 0., name = None):
...
class WindowAttention(tf.keras.Model):
    ...
    def __init__(self, filters, window_size, num_heads, use_qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop = 0., name = None):
    ...
    def call(self, x, attention_mask = None, training = None):
        ...
        if attention_mask is not None:
            ...
            attn = tf.reshape(attn, shape=[-1, nW, self.num_heads, N, N]) + tf.cast(tf.expand_dims(tf.expand_dims(attention_mask, axis=1), axis=0), attn.dtype)
...
class SwinTransformerBlock(tf.keras.Model):
    ...
    def __init__(self, filters,  
        ...
        self.attention = WindowAttention(filters, window_size = (self.window_size, self.window_size), 
                                                  num_heads = num_heads,
        ...
        self.mlp = Mlp(in_features = filters, 
...
class PatchMerging(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        x = tf.reshape(x, shape=[-1, (input_height // 2) * (input_width // 2), 4 * channels], name = "reshape_half_size_to_channel")
...
class BasicLayer(tf.keras.Model):
    ...
    def __init__(self, filters,
        ...
        for i in range(depth):
            block = SwinTransformerBlock(filters = filters,
                                         num_heads = num_heads, 
    ...
    def generate_attention_mask (self, input_height, input_width, window_size, shift_size):
        ...
        for h in h_lens:
            ...
            for w in w_lens:
                p = tf.ones(shape = [1, h, w, 1], dtype = tf.int16)
                p *= cnt
                w_masks.append(p)
...
class PatchEmbed(tf.keras.Model):
    def __init__(self, patch_size = (4, 4), 
        ...
        self.proj = tf.keras.layers.Conv2D(embed_filters, kernel_size = patch_size,
                                                          strides = patch_size, 
                                                          name = f"{name}/proj")
    ...
    def call(self, x):
        ...
        x = tf.reshape(x, shape=[-1, (padded_height // self.patch_size[0]), (padded_width // self.patch_size[0]), self.embed_dim])
...
class SwinTransformerModel(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        if self.use_absolute_pos_embed:
            self.absolute_pos_embed = self.add_weight(name = 'absolute_pos_embed',
        ...
        for i_layer in range(self.num_layers):
            basic_layer = BasicLayer(filters = int(self.embed_dim * 2 ** i_layer),
...
def swin_tiny_224(return_endpoints = False):
    ...
    return SwinTransformerModel(name = "swin_tiny_224",
                                window_size = 7,
                                embed_dim = 96,
                                depths = [2, 2, 6, 2],
                                num_heads = [3, 6, 12, 24],
...
def swin_base_384(return_endpoints = False):
    ...
    return SwinTransformerModel(name = "swin_base_384",
                                window_size = 12,
                                embed_dim=  128,
                                depths = [2, 2, 18, 2],
                                num_heads = [4, 8, 16, 32],
...
def swin_large_384(return_endpoints = False):
    ...
    return SwinTransformerModel(name = "swin_large_384",
                                window_size = 12,
                                embed_dim = 192,
                                depths = [2, 2, 18, 2],
